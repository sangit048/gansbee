...
// Helper: gắn variants + convert image URL đầy đủ cho cả product và variant
async function attachVariantsWithImages(product, BASE_URL) {
  const variants = await ProductVariant.find({ product: product._id }).lean();
  ...
  return { ...product, images, variants: variantsWithFullImage };
}

// ─────────────────────────────────────────────
// GET /images/:filename — Trả ảnh ra FE (dùng chung cho product & variant)
// ─────────────────────────────────────────────
router.get("/images/:filename", async (req, res) => {
  try {
    const bucket = getBucket();
...
router.get("/", async (req, res) => {
  try {
    ...
    const productsWithVariants = await Promise.all(
      products.map((p) => attachVariantsWithImages(p, BASE_URL)),
    );
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const result = await attachVariantsWithImages(product, BASE_URL);
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    // Xoá luôn ảnh của variants thuộc product này
